...
from .provider_validator import AIProviderValidator, ValidationResult, ModelTestResult
...
class AIProviderManager:
    ...
    def _load_providers(self) -> None:
        ...
        try:
            ...
            print(f"Warning: Failed to load provider configurations from {self._config_file}: {e}")
            self._providers = {}
    ...
    def _save_providers(self) -> None:
        ...
        try:
            ...
            for provider_id, provider in self._providers.items():
                provider_dict = {
                    ...
                    'config': {
                        ...
                        'models': {
                            'small': provider.config.models.small.model_dump() if provider.config.models.small else None,
                            'main': provider.config.models.main.model_dump() if provider.config.models.main else None
                        },
            ...
            print(f"Warning: Failed to save provider configurations to {self._config_file}: {e}")
    ...
    async def update_provider(self, provider_id: str, config: ProviderConfig) -> ProviderUpdateResponse:
        """
        ...
        for other_provider in self._providers.values():
            if other_provider.id != provider_id and other_provider.config.name == config.name:
                raise MCPError(f"Provider with name '{config.name}' already exists")
    ...
    async def add_model(self, provider_id: str, model_name: str, config: ModelConfig) -> ModelCreateResponse:
        """
        ...
        if model_name in provider.config.models:
            raise MCPError(f"Model '{model_name}' already exists for provider '{provider.config.name}'")
    ...
    async def update_model(self, provider_id: str, model_name: str, config: ModelConfig) -> ModelUpdateResponse:
        """
        ...
        if model_name not in provider.config.models:
            raise MCPError(f"Model '{model_name}' not found for provider '{provider.config.name}'")
    ...
    async def delete_model(self, provider_id: str, model_name: str) -> ModelDeleteResponse:
        """
    ...
    async def set_model(self, provider_id: str, model_type: str, model_config: ModelConfig) -> ModelSetResponse:
        """
        ...
        if model_type not in ['small', 'main']:
            raise MCPError(f"Invalid model type '{model_type}'. Must be 'small' or 'main'")
    ...
    async def remove_model(self, provider_id: str, model_type: str) -> ModelDeleteResponse:
        """
        ...
        if model_type == 'small' and not provider.config.models.small:
            raise MCPError(f"No small model configured for provider '{provider.config.name}'")
        elif model_type == 'main' and not provider.config.models.main:
            raise MCPError(f"No main model configured for provider '{provider.config.name}'")
    ...
    async def validate_provider(self, provider_id: str, model_name: Optional[str] = None) -> ValidationResult:
        """
    ...
    async def test_model(self, provider_id: str, model_name: str, test_message: str = "Hello, this is a test.") -> ModelTestResult:
        """
        ...
        return await self._validator.test_model(provider.config, model_name, test_message)