...
from mcp_open_client.api.models.chat import (
    ChatRequest, ChatResponse, ModelType, Message, Choice, Usage, ToolCall
)
...
class ChatService:
    ...
    def _resolve_provider_and_model(
        ...
        if request.provider:
            ...
            if not provider_name:
                raise ValueError("No default provider configured. Please set a default provider or specify one in the request.")
    ...
    def _convert_messages_for_openai(self, messages: List[Message]) -> List[Dict[str, Any]]:
        """
    ...
    def _convert_function_call(self, function_call: Union[str, Dict[str, str]]) -> Union[str, Dict[str, Any]]:
        """
    ...
    def _get_mcp_tools(self, server_names: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            running_servers = [server for server in servers if server.status.value == "running"]
            
            # Filter by server_names if provided
            if server_names:
                running_servers = [server for server in running_servers if server.config.name in server_names]
            ...
            for server in running_servers:
                try:
                    ...
                    tools_response = self.server_manager.get_server_tools(server.config.name)
                    if tools_response.success:
                        # Convert each tool to OpenAI function format
                        for tool in tools_response.tools:
                            function = self._convert_tool_to_function(tool, server.config.name)
                            functions.append(function)
                except Exception as e:
                    # Log error but continue with other servers
                    print(f"Warning: Could not get tools from server '{server.config.name}': {str(e)}")
                    continue
    ...
    def _convert_tool_to_function(self, tool: ToolInfo, server_name: str) -> Dict[str, Any]:
        """
    ...
    def chat_completion(self, request: ChatRequest) -> ChatResponse:
        ...
        provider_name, actual_model_name, model_type = self._resolve_provider_and_model(request)
        ...
        if request.function_call:
            chat_params["function_call"] = self._convert_function_call(request.function_call)
        ...
        try:
            ...
            return self._convert_response(response, actual_model_name, provider_name, model_type)
